...
const PHOTO_INSTRUCTION = `The user has sent a PHOTO instead of (or alongside) a text description. Identify what is on the plate, following the same naming/quantity/confidence rules as the text case above.

CRITICAL — name DISHES, not ingredients. Report one item per distinct *preparation*, not per ingredient you can see in it. If several foods have been cooked together into one dish, that is ONE item, named as the dish: a pan of peas, tomato and capsicum cooked together is "mixed vegetable sabzi" (one item), NOT "green peas" + "tomatoes" + "capsicum" (three items). Only list items separately when they are physically separate portions — the different bowls of a thali, or rice beside a curry beside a roti.

This matters because a cooked dish is not the sum of its raw parts: it carries cooking oil or ghee, which is invisible in a photo but is often a third of the calories. Naming raw ingredients prices the meal as if it were a salad. When in doubt, prefer the dish name. Estimate each quantity/weight from typical portion sizes and visual scale relative to the plate/container — never leave quantity or unit blank. Confidence should reflect how clearly each item is visually identifiable, using the same 0-1 calibration described above (a clearly-recognizable specific dish is high; something partially obscured, a generic-looking gravy/curry with no visible identifying ingredient, or a food you're genuinely unsure of is medium or low — never guess a specific name you can't actually see support for in the image). If the photo is not of food at all (e.g. blurry, unrelated), return a single low-confidence item named "unclear photo" rather than fabricating a dish.
